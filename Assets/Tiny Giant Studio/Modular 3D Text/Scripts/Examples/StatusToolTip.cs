...
namespace MText
{
    public class StatusToolTip : MonoBehaviour
    {
        ...
        private GameObject GetNewObj(int style)
        {
            ...
            if (pooling) 
                newItem.AddComponent<MText_PoolItem>().key = styles[style].textPrefab.name;
        ...
        IEnumerator DestroyObj(GameObject obj, float duration)
        {
            ...
            if (pooling)
            {
                if (obj.GetComponent<MText_PoolItem>())
                {
                    if (poolDictionary.ContainsKey(obj.GetComponent<MText_PoolItem>().key))
                    {
                        obj.SetActive(false);
                        obj.transform.SetParent(transform);
                        poolDictionary[obj.GetComponent<MText_PoolItem>().key].Enqueue(obj);
                    }
                    else Destroy(obj);
                }
                else Destroy(obj);
